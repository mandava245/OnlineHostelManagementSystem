...
<%
	...
	if(request.getParameter("hidden")!=null){
		gid = request.getParameter("pergroc");
		if(gid!=null){
		...
		String query = "SELECT tname,quantity,price,total FROM groceries WHERE gid='"+gid+"'";
	...
	%>
		...
		<div align="center">
			<form action="grocenter.jsp" method="post">
			<table align="center" id="second" border=0 cellpadding=5 cellspacing=5>
				<tr><td>New Quantity : </td><td> <%= vquantity %></td></tr>
				<tr><td>New Price : </td><td> <%= vprice %></td></tr>
			</table><br>
				<input type="hidden" name="hidden" value="hidden" />
				<input type="hidden" name="gid" value="<%= gid %>" />
				<input type="hidden" name="quantity" value="<%= vquantity %>" />
