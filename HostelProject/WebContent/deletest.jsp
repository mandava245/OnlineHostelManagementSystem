...
<title>DELETE STUDENTS</title>
		...
		<script>
				function formConfirm(){
				    ...
				    //  var chkall = document.chkFrm.checkall.value;
				    var count = 0;
					...
					var cnf = confirm('Are you sure you want to Delete '+count+' Students Data?');
					if(cnf==true){
						return true;
					}
					else{
...
<script>
	...
	<form action="deletest.jsp" method="post">
	...
	<%
		...
		{
			if(branch==null || branch.equals("all")){
			sql = "SELECT * FROM student ORDER BY syear,roomno ASC";
			}
			else
			{
				sql = "SELECT * FROM student WHERE branch='"+branch+"' ORDER BY syear,roomno ASC";
			}
		}
		else if(branch==null || branch.equals("all")){
			sql = "SELECT * FROM student WHERE syear="+year+" ORDER BY roomno ASC";
		}
	...
	%>
	<form name="chkFrm" action="delstcnf.jsp" method="post" onsubmit="return formConfirm();">
	<table align="center" border=1>
		<tr>
			<th width=60px><input type="checkbox" name="checkall" value="Pavan" id="checkall" onchange="checkedAll(this)" /></th>
		...
		%>
		<tr>
			<td><input type="checkbox" class="check" id="check" name="<%= sid %>" value="<%= sid %>" /></td>
